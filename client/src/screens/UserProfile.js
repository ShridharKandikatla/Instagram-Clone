...
const UserProfilePage = () => {
  ...
  const [showFollow, setShowFollow] = useState(
    state ? !state.Following.includes(userid) : null
  );
  ...
  useEffect(() => {
    axios
      .get(`https://instagram-clone-qrc9.onrender.com/user/${userid}`, config)
      .then((res) => {
        setData(res.data);
      });
  }, []);
  ...
  const followUser = () => {
    axios
      .put(
        `https://instagram-clone-qrc9.onrender.com/follow`,
        { followId: userid },
        config
      )
      .then((result) => {
  ...
  const unfollowUser = () => {
    axios
      .put(
        `https://instagram-clone-qrc9.onrender.com/unfollow`,
        { unfollowId: userid },
        config
      )
      .then((result) => {
