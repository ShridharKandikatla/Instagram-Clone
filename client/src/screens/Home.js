...
const Home = () => {
   ...
   const likePost = (id) => {
      axios
         .put(`http://localhost:5000/like`, { postId: id }, config)
         .then((result) => {
   ...
   const unlikePost = (id) => {
      axios
         .put(`http://localhost:5000/Unlike`, { postId: id }, config)
         .then((res) => {
            const newData = data.map((item) => {
   ...
   const bookmark = (id) => {
      axios
         .put(`http://localhost:5000/bookmark-post`, { postId: id }, config)
         .then((result) => {
            dispatch({
               type: BOOKMARK_POST,
   ...
   const removeBookmark = (id) => {
      axios
         .put(`http://localhost:5000/remove-bookmark`, { postId: id }, config)
         .then((result) => {
            dispatch({
               type: BOOKMARK_POST,
               payload: { Bookmarks: result.data.Bookmarks },
   ...
   const makeComment = (text, postId) => {
      setComment("");
      axios
         .put(`http://localhost:5000/comment`, { text, postId }, config)
         .then((result) => {
            const newData = data.map((item) => {
               if (result.data._id === item._id) return result.data;
               else return item;
   ...
   const deletePost = (postId) => {
      axios
         .delete(`http://localhost:5000/deletepost/${postId}`, config)
         .then((res) => {
